...
const SettingsPage = () => {
    ...
    if (loading) return <div className="settings-loading"><div className="spinner"></div></div>;
    if (!settings) return <div className="settings-loading"><p>Failed to load settings</p></div>;
                ...
                {activeTab === 'hours' && (
                    <div className="hours-section">
                        <div className="section-info">
                            ...
                            <p>Define the operating hours for each day. This helps in calculating shift coverage.</p>
                        </div>

                        <div className="hours-grid">
                            {settings.openingHours && Object.entries(settings.openingHours).map(([day, hours]) => (
                                <div key={day} className={`day-row ${hours.closed ? 'closed' : ''}`}>
                            ...
                            {!settings.openingHours && <p>No opening hours configuration found.</p>}
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'requirements' && (
                    <div className="requirements-section">
                        <div className="section-info">
                            ...
                            <p>Set the minimum number of staff and total hours needed per position daily.</p>
                        </div>

                        <div className="requirements-grid">
                            {settings.requirements && Object.entries(settings.requirements).map(([position, req]) => (
                                <div key={position} className="requirement-card">
                                    ...
                                    <div className="req-input-group">
                                        ...
                                        <input
                                            ...
                                            min="0"
                                            value={req.minCount}
                                            onChange={(e) => handleRequirementChange(position, 'minCount', e.target.value)}
                                        />
                                    </div>
                                    ...
                                    <div className="req-input-group">
                                        ...
                                        <input
                                            ...
                                            min="0"
                                            value={req.minHours}
                                            onChange={(e) => handleRequirementChange(position, 'minHours', e.target.value)}
                                        />
                                    </div>
                                </div>
                            ))}
