                            ...
                            {dayShifts.length > 0 && (
                                <div className="shift-indicators">
                                    {dayShifts.slice(0, 3).map((shift) => (
                                        <div key={shift.id} className="shift-indicator" title={`${shift.employeeName}: ${shift.startTime}-${shift.endTime}`}>
                                            <span className="shift-name">{shift.employeeName}</span>
                                            <span className="shift-time">{shift.startTime}</span>
                                        </div>
                                    ))}
                                    {dayShifts.length > 3 && (
                                        <div className="more-shifts">+{dayShifts.length - 3} more</div>
                                    )}
                                </div>
                            )}
                        </div>
