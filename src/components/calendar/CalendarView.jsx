...
const CalendarView = () => {
    ...
    const renderView = () => {
        switch (viewMode) {
            ...
            case VIEW_MODES.MONTHLY:
                return <MonthlyView currentDate={currentDate} onDateSelect={setCurrentDate} />;
            case VIEW_MODES.YEARLY:
                return <YearlyView currentDate={currentDate} onMonthSelect={(date) => {
                    setCurrentDate(date);
        ...
        <div className="calendar-view">
            <div className="calendar-header">
                ...
                <div className="view-switcher">
                    ...
                    <button
                        className={viewMode === VIEW_MODES.DAILY ? 'view-button active' : 'view-button'}
                        onClick={() => setViewMode(VIEW_MODES.DAILY)}
                    >
                        Day
                    </button>
                    <button
                        className={viewMode === VIEW_MODES.WEEKLY ? 'view-button active' : 'view-button'}
                        onClick={() => setViewMode(VIEW_MODES.WEEKLY)}
                    >
                        Week
                    </button>
                    <button
                        className={viewMode === VIEW_MODES.MONTHLY ? 'view-button active' : 'view-button'}
                        onClick={() => setViewMode(VIEW_MODES.MONTHLY)}
                    >
                        Month
                    </button>
                    <button
                        className={viewMode === VIEW_MODES.YEARLY ? 'view-button active' : 'view-button'}
