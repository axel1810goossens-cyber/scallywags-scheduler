...
const ShiftForm = ({ shift, defaultDate, onClose }) => {
  ...
  useEffect(() => {
    if (shift) {
      // Initialize form data from shift
      const initFormData = {
  ...
  const filteredEmployees = employees.filter(emp => {
    const matchesPosition =
      !formData.position || emp.position === formData.position;
  ...
  const handleSubmit = async e => {
    ...
    const dayOfWeek = dateHelpers
      .getDayName(dateHelpers.parseDate(formData.date))
      .toLowerCase();
    ...
    if (conflict.hasConflict) {
      if (
        !window.confirm(`Warning: ${conflict.message}. Do you want to proceed?`)
      ) {
        return;
      }
    }
                ...
                <div className="employee-dropdown">
                  {filteredEmployees.length > 0 ? (
                    ...
                    <div className="no-employees">
                      {employeeSearch
                        ? 'No employees found'
                        : `No ${formData.position}s available`}
                    </div>
                  )}
                </div>
              )}
