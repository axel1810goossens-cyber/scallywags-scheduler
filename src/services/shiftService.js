...
export const shiftService = {
    ...
    updateShift: async (id, updates) => {
        if (!isFirebaseConfigured()) {
            ...
            return success ? { success: true } : { success: false, error: 'Shift not found' };
        }
    ...
    subscribeToShifts: (startDate, endDate, callback) => {
        ...
        if (!isFirebaseConfigured()) {
            console.log('Using localStorage for shifts, date range:', startDate, 'to', endDate);

            // Initial call
            const filteredShifts = localStorageService.getShiftsByDateRange(startDate, endDate);
            callback(filteredShifts);

            // Set up polling to detect changes (every 500ms)
            const interval = setInterval(() => {
                const shifts = localStorageService.getShiftsByDateRange(startDate, endDate);
                callback(shifts);
            }, 500);
