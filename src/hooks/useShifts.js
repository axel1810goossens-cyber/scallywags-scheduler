...
export const useShifts = (startDate, endDate) => {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        const unsubscribe = shiftService.subscribeToShifts(startDate, endDate, (data) => {
            setShifts(data);
            setLoading(false);
            setError(null);
        });
    ...
    const swapShifts = async (shift1Id, shift2Id, employee1Data, employee2Data) => {
        const result = await shiftService.swapShifts(shift1Id, shift2Id, employee1Data, employee2Data);
        if (!result.success) {
            setError(result.error);
        }
        return result;
