export const validators = {
    ...
    checkAvailabilityConflict: (employee, dayOfWeek, startTime, endTime) => {
        if (!employee.availability || !employee.availability[dayOfWeek]) {
            return { hasConflict: true, message: 'Employee not available on this day' };
        }

        const availableSlots = employee.availability[dayOfWeek];

        for (const slot of availableSlots) {
            if (validators.isTimeWithinRange(startTime, endTime, slot.start, slot.end)) {
                return { hasConflict: false };
            }
        }

        return { hasConflict: true, message: 'Shift time outside employee availability' };
    },
    ...
    isTimeWithinRange: (checkStart, checkEnd, rangeStart, rangeEnd) => {
        ...
        return checkStartMinutes >= rangeStartMinutes && checkEndMinutes <= rangeEndMinutes;
    },
