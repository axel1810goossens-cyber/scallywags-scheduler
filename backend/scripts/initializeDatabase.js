...
const loadMockData = () => {
  try {
    ...
    const employeesMatch = mockDataContent.match(
      /export const mockEmployees = (\[[\s\S]*?\n\]);/
    );
    // Extract shifts array
    const shiftsMatch = mockDataContent.match(
      /export const mockShifts = (\[[\s\S]*?\]);/
    );
    ...
    console.error(
      '⚠️  Could not load mock data from mockData.js:',
      error.message
    );
  ...
  try {
    ...
    if (settingsDoc.exists) {
      ...
      await settingsRef.set({
        ...
        positions: ['Server', 'Bartender', 'Manager', 'Kitchen', 'Host'],
        minShiftDuration: 4,
        maxShiftDuration: 12,
        maxWeeklyHours: 40,
        createdAt: new Date().toISOString(),
    ...
    if (!employeesSnapshot.empty) {
      ...
      if (mockData && mockData.mockEmployees) {
        console.log(
          `📝 Creating ${mockData.mockEmployees.length} employees from mockData.js...`
        );
    ...
    if (!shiftsSnapshot.empty) {
      ...
      if (mockData && mockData.mockShifts && mockData.employeeIdMap) {
        console.log(
          `📝 Creating ${mockData.mockShifts.length} shifts from mockData.js...`
        );
    ...
    if (mockData) {
      console.log(
        `   • ${mockData.mockEmployees?.length || 0} employees loaded`
      );
